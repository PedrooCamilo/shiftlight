//Este código foi obtido no github oficial da placa BitDogLab
...
// Para funcionalidades futuras (estado dos botões para aumentar o rpm base)
typedef enum
{
  IDLE,         // Estado inativo
...
{
  play_note(pin, melody[0]); // Toca a nota
    sleep_ms(200);             // Define a duração do beep (200ms)
    play_rest(pin);     
}
